...
export function NoteEditor({
  ...
  async function flushSave(currentTitle: string, currentContent: string) {
    ...
    try {
      ...
      const saved = localNoteId ? await updateNote(localNoteId, draft) : await createNote(projectId, draft);
      if (!localNoteId) setLocalNoteId(saved.id);
      const names = extractTagNames(saved.content);
      if (names.length > 0) await ensureNoteTagsExist(projectId, names, noteTags);
      onSaved();
      setSaveStatus("saved");
      setErr(null);
    } catch (e: any) {
