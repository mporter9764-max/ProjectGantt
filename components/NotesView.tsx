...
import type { Note, NoteTag, NoteSnippetCompletion, NoteLink, Task } from "@/lib/types";
...
export function NotesView({
  ...
  onManageTags: () => void;
  onSendToChart: (noteId: string, tag: string, hash: string, text: string) => void;
  ...
  const linkMap = useMemo(
    () => new Map(links.map((l) => [`${l.note_id}|${l.tag}|${l.snippet_hash}`, l])),
    [links]
  );
  const isDone = (noteId: string, tag: string, hash: string) => {
    const key = `${noteId}|${tag}|${hash}`;
    if (doneSet.has(key)) return true;
    const link = linkMap.get(key);
    return Boolean(link && tasksById.get(link.task_id)?.is_complete);
  };
  ...
  const openSnippets = useMemo(
    () => allSnippets.filter((s) => !isDone(s.noteId, s.tag, s.hash)),
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [allSnippets, doneSet, linkMap, tasksById]
  );
                ...
                .map((t) => {
                  ...
                  <div key={tag}>
                    ...
                    <div className="space-y-2">
                      {snippets.map((s, i) => {
                        const done = isDone(s.noteId, s.tag, s.hash);
                        const link = linkMap.get(`${s.noteId}|${s.tag}|${s.hash}`);
                        const linkedTask = link ? tasksById.get(link.task_id) : undefined;
                        return (
                          <TaggedSnippetRow
                            key={i}
                            text={s.text}
                            ...
                            chip={
                              linkedTask ? (
                                <span className="mt-1 inline-flex items-center gap-1 rounded-full bg-black/[0.05] px-2 py-0.5 text-[10px] font-medium text-muted">
                                  ⛳ On chart: {linkedTask.title.slice(0, 30)} · {linkedTask.is_complete ? "done" : linkedTask.start_date}
                                </span>
                              ) : undefined
                            }
                            onToggleDone={() =>
                            ...
                            onSendToTask={() => onSendToChart(s.noteId, s.tag, s.hash, s.text)}
                          />
                        );
                      })}
                    </div>
